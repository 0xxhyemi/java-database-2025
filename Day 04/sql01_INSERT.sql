...
-- 컬럼리스트 순서와 갯수가 다를떄는 반드시 적어줘야 함. 
INSERT INTO new_table (jumin, name, NO)
VALUES ('760921-1825899', '성유고', 3);

-- 값이 먼지 모를때는 NULL로 삽입
INSERT INTO new_table
VALUES (4, '홍길태', ' 830108-1825699', NULL, NULL);

-- 한테이블에 있는 데이터를 모두 옮기면서 새로운 테이블 생성
-- PK는 복사가 안됨!
CREATE TABLE professor_new
AS
 SELECT * FROM professor;

SELECT * FROM PREOFESSOR_NEW;

-- 만들어진 테이블에 데이터 한꺼번에 옮기기
INSERT INTO PREOFESSOR_NEW
SELECT * FROM PROFESSOR;

--새로 만들어진 테이블 Professor_new 데이터를 삽입 테스트
INSERT INTO PREOFESSOR_NEW (profno, name, id, POSITION, pay, hiredate)
VALUES (4008, 'Tom Cruise', 'Cruise', 'instructor', 300,'2025-03-14');

-- PROCFESSOR_NEW는 PK가 없기 때문에 같은 값이 들어감
INSERT INTO PREOFESSOR_NEW (profno, name, id, POSITION, pay, hiredate)
