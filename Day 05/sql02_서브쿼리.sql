...
-- 지역이 PohangMain Office인 부서코드에 해당하는 직원들의 사번, 이름, 부서번호를 출력하시오.
...
-- emp2 테이블에서 'Selection head' 직급의 최소 연봉보다 연봉이 높은 사람의 이름, 직급, 연봉을 출력하시오.
...
-- emp2와 위에서 구한 값을 조인해서 deptno별 평균연봉보다 얼마씩 차이 나는지
...
-- 위의 쿼리 조인으로 변경가능
SELECT e.name, d.deptno, nvl(d.dname, '부서명없음') AS "부서명"
  FROM emp2 e, dept2 d
 WHERE e.deptno = d.dcode(+)
 ORDER BY deptno, name;
