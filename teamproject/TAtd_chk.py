...
class TAtdMainWindow(QMainWindow): 
    ...
    def numchkClick(self):
        ...
        try:
            ...
            '''
            cursor.execute(query_1, {'random_number': random_number})
            ...
            WHERE T_NO IN (
                SELECT T_NO
                FROM ATD
                WHERE CHECKNO = :random_number
            )
        '''
            cursor.execute(query_2, {'random_number': random_number})
        ...
        pixmap = QPixmap('./image/다운로드 (6).png')  # 이미지 파일 경로
